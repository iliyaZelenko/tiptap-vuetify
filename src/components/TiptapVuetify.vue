  ...
  >
    ...
    <VCard v-if="$props[PROPS.TYPE] === EDITOR_TYPES_ENUM.card" v-bind="$props[PROPS.CARD_PROPS]">
      ...
      >
        ...
        <template v-if="$scopedSlots.toolbar" #default="scopedSlot">
          <slot name="toolbar" v-bind="scopedSlot" />
        </template>
      </toolbar>
...
export default class TiptapVuetify extends Vue {
  ...
  mounted() {
    ...
    this[PROPS.EXTENSIONS].forEach(extensionDefinition => {
      ...
      this.availableActions[paramsFinal.renderIn].push(
    ...
    extensionsInstances.forEach(
      ext => ext.onEditorInit && ext.onEditorInit(this.editor!)
    );
  }
